...
export default {
    ...
    execute: async ({ interaction }) => {
        ...
        const discussionDescriptionInput = new TextInputBuilder()
            ...
            .setPlaceholder("I think this bot is awesome because it's open source and it's free!")
            .setMaxLength(1000)
            .setMinLength(10);

        const row = new ActionRowBuilder().addComponents(discussionTitleInput);
        const row2 = new ActionRowBuilder().addComponents(discussionDescriptionInput);

        discussionModal.addComponents(row, row2);

        await interaction.showModal(discussionModal).catch((e) => console.log(e));
    },
};
