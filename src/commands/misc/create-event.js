...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('duration')
        ...
        .setRequired(true)
        .addChoices({ name: '1 minute', value: 'min' }, { name: '1 hour', value: 'hour' }, { name: '1 day', value: 'day' })
    ),
  async execute({ interaction }) {
    ...
    const scheduledEndTime = moment(scheduledStartTime).add(30, 'minutes');
    ...
    return interaction.editReply({
      content: `Created an event starting at ${scheduledStartTime.format('lll')} and ending at ${scheduledEndTime.format('lll')} in ${timezone}`
    });
  }
}
